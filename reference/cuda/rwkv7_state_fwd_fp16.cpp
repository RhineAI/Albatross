...
void cuda_spmv_forward(F* vec, F* mat, F* out);
...
void spmv_forward(torch::Tensor &vec, torch::Tensor &mat, torch::Tensor &out) {
    cuda_spmv_forward(vec.data_ptr<F>(), mat.data_ptr<F>(), out.data_ptr<F>());
}
